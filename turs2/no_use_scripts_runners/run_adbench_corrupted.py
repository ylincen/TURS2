...
# random flip some 0 to 1,
np.random.seed(0)
flip_rate = 0.2
flip_idx = np.random.choice(np.where(y == 0)[0], int(len(y) * flip_rate), replace=False)
y[flip_idx] = 1
...
folder_name = "exp_adbench_corrupted" + date_and_time[:8]
# if not_use_excl_:
#     folder_name = "exp_uci_NotUseExcl" + date_and_time[:8]
# elif validity_check_ == "none":
#     folder_name = "exp_uci_NoValidCheck" + date_and_time[:8]
# else:
#     folder_name = "exp_uci_" + date_and_time[:8]

# write the not_use_excl_ and validity_check_ as the content of a README file
os.makedirs(folder_name, exist_ok=True)

with open("./" + folder_name + "/README.txt", "w") as f:
    f.write("not_use_excl_: " + str(not_use_excl_) + "\n")
    f.write("validity_check_: " + str(validity_check_) + "\n")
