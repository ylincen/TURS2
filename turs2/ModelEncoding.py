    ...
    def rule_cl_model_dep(self, condition_matrix, col_orders):
        ...
        for index, col in enumerate(col_orders):
            if np.count_nonzero(bool_) == 0:
                ...
                bits_direction = 0
                bits_one_or_two_cuts = 0
                if condition_count[col] == 1:
                    if num_cuts == 0:
                        l_cuts += 0 + bits_one_or_two_cuts + bits_direction
                    else:
                        l_cuts += np.log2(num_cuts) + bits_one_or_two_cuts + bits_direction
                else:
                    if num_cuts >= 2:
                        l_cuts += np.log2(num_cuts) + np.log2(num_cuts - 1) - np.log2(2) + bits_one_or_two_cuts
                    else:
                        l_cuts += 0 + bits_one_or_two_cuts

                if index != len(col_orders) - 1: # if we've reached the last condition, no need to filter data points anymore
