    ...
    def update_grow_beam(self, bi_array, excl_bi_array, icol, cut, cut_option, incl_coverage, excl_coverage,
        ...
        if mode == "normalized":
            ...
            grow_info = store_grow_info(
                ...
                excl_mdl_gain=info_theo_scores["absolute_gain_excl"],
                coverage_excl=excl_coverage, coverage_incl=incl_coverage,
                normalized_gain_excl=info_theo_scores["absolute_gain_excl"],
                normalized_gain_incl=info_theo_scores["absolute_gain"],
                _rule=self
            )
    ...
    def grow(self, grow_info_beam, grow_info_beam_excl):
        ...
        for icol in range(self.ncol):
            ...
            for i, cut in enumerate(candidate_cuts_icol):
                # Explanations for what to skip

                ## if it is "either", we check for skipping points; otherwise it must be "none"
                # print("self.data_info.alg_config.validity_check: ", self.data_info.alg_config.validity_check)
                if self.data_info.alg_config.validity_check == "either":
                    _validity = validity_check(rule=self, icol=icol, cut=cut)
                else:
                    assert self.data_info.alg_config.validity_check == "none"
                    _validity = {"res_excl": True, "res_incl": True}
                

                ## if we set "not_use_excl_" as true, we set all res_excl to false to skip all "excl" calculations
                if self.data_info.not_use_excl_:
                    _validity["res_excl"] = False

                ## skip the whole search if both excl and incl are invalid
                ...
                if (excl_left_coverage < self.data_info.alg_config.min_sample_each_rule or
                        excl_right_coverage < self.data_info.alg_config.min_sample_each_rule):
                    continue
