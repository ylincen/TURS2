...
d$sig_Bonferroni = sig_Bonferroni
d$sig_HonnBonferroni = sig_HonnBonferroni
dt = data.table(d)
dt = dt[, .(sig_perc = mean(overlap_significances_perc,na.rm=T)), by=.(data_name)]

library(ggplot2)
# ggplot(dt) + geom_col(aes(x=data_name, y=sig_perc)) + 
#     theme(axis.text.x = element_text(angle = 90, hjust = 1))

ggplot(d) + geom_boxplot(aes(x=data_name, y=overlap_significances_perc)) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  ylim(c(0,1)) + labs(y="% of significant overlaps", x="")
ggsave("~/projects/TURS/turs2/paper_jmlr/overlap_sig.png",
       device = "png", width=25, height=15, units="cm", dpi=300)


# df <- do.call(rbind, mapply(function(pvalues, dataset) {
#   data.frame(dataset, pvalue = pvalues)
# }, pvalues = pvalues_all, dataset = d$data_name, SIMPLIFY = F))
# ggplot(df) + geom_boxplot(aes(x=dataset, y=pvalue))

# widths <- lapply(pvalues_all, function(x) 1 / length(x))
# 
# df <- do.call(rbind, mapply(function(pvalues, dataset, width) {
#   data.frame(dataset, pvalue = pvalues, width = width)
# }, pvalues = pvalues_all, dataset = d$data_name, 
# width = widths, SIMPLIFY = FALSE))
# df$rule_index = unlist(lapply(pvalues_all, function(l) (1:length(l)).length(l)) )

# ggplot(df) +
#   geom_tile(aes(x = dataset, y = rule_index-width/2, fill = pvalue,
#                 alpha=1, height=width)) +
#   scale_fill_gradient(low = "skyblue", high = "blue", name = "P-value", na.value = "black") +
#   labs(title = "Adaptive Width Heatmap of p-values", x = "Dataset", y = "Observation Index") +
