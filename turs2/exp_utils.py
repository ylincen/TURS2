...
def cover_matrix_fun(ruleset, X):
    # This function gets the cover matrix of a rule set and a feature matrix X;
    #  specifically, a cover matrix $M$ contains binary elements $M(i,j)$, which denote whether the $i$-th data point
    #  is covered by the $j$-th rule in the rule set;
    ...
    for ir, rule in enumerate(ruleset.rules):
        ...
        r_bool_array = get_rule_cover(rule, X)
...
def explainability_analysis(ruleset, X, cover_mat):
    # This function, perhaps badly named during the implementation, calculates the average number of literals a domain
    # expert needs to go through to explain the prediction of a single data point.

    assert cover_mat.shape[1] == len(ruleset.rules) + 1
    assert cover_mat.shape[0] == len(X)

    cover_mat_NoElseRule = cover_mat[:, :-1]
    ...
    if all(num_literals_each_data == 0):
        return 0.0
    else:
        return np.mean(num_literals_each_data[num_literals_each_data != 0])  # exclude data points covered by else-rule;

def overlap_analysis(ruleset, cover_mat):
    # This function calculates the differences of the predicted target probs. given by different rules for the same data point.

    # get the unique rows of cover_mat
    cover_mat_unique, unique_counts = np.unique(cover_mat, axis=0, return_counts=True)
    assert cover_mat_unique.shape[1] == len(ruleset.rules) + 1

    # for each unique row, calculate the difference of the predicted target probs. given by different rules for the same data point.
    diff_probs_mean = []  # this is for each rule/overlap;
    diff_probs_max = []
    rules_prob = [r.prob for r in ruleset.rules] + [ruleset.else_rule_p]
    rules_prob = np.array(rules_prob, dtype=float)
    for irow, row in enumerate(cover_mat_unique):
        irules = np.where(row)[0]
        if len(irules) > 1:
            rules_prob_matrix_ = rules_prob[irules]
            diffs_ = np.zeros(rules_prob_matrix_.shape[1], dtype=float)
            for icol in range(rules_prob_matrix_.shape[1]):
                max_p = np.max(rules_prob_matrix_[:, icol])
                min_p = np.min(rules_prob_matrix_[:, icol])
                diffs_[icol] = max_p - min_p
            diff_probs_mean.append(np.mean(diffs_))
            diff_probs_max.append(np.max(diffs_))
        else:
            diff_probs_mean.append(np.nan)
            diff_probs_max.append(np.nan)
    return {"diff_probs_mean": diff_probs_mean, "diff_probs_max": diff_probs_max,
            "modelling_group_counts": unique_counts}


def predict_random_picking_for_overlaps(ruleset, X, seed, cover_mat):
    np.random.seed(seed)

    # cover_mat = cover_matrix_fun(ruleset, X)
...
def calculate_overlap_percentage(ruleset, X, cover_mat):
    # cover_mat = cover_matrix_fun(ruleset, X)
