...
for(ii in 1:length(data_names)){
  if(data_names[ii] == "3_backdoor.npz"){
    next()
    # Will give error:
    # fold:  0
    # Error in .jcall(o, "Ljava/lang/Class;", "getClass") :
    #   java.lang.OutOfMemoryError: Java heap space
    # Calls: JRip ... .predictions_for_instances -> .has_method -> .jcall -> .jcheck
  }
  ...
  for(fold in 0:4){
    ...
    start_time = Sys.time()
    model = JRip(Ytrain~., data = d_train)
  ...
  write.csv(res_df, "~/projects/TURS/turs2/ad_bench_competitors/RIPPER/res_RIPPER_noise20percent.csv")
}
