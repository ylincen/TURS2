...
for data_name in datasets_without_header_row + datasets_with_header_row:
    ...
    for fold in range(5):
        ...
        rulelist = Ruleset(data_info=data_info, features=data_info.features, target=data_info.target,
                           number_of_init_rules=number_of_init_rules)

        # Grow rules;
        t0 = time.time()
        rulelist.build_rule_list(max_iter=1000, beam_width=beam_width, candidate_cuts=data_info.candidate_cuts)

        X_test = dtest.iloc[:, :dtest.shape[1]-1].to_numpy()
        y_test = dtest.iloc[:, dtest.shape[1]-1].to_numpy()
        test_p = get_test_p_rulelist(rulelist, X_test)
        ...
        train_p = get_test_p_rulelist(rulelist, X_train)
        if len(test_p[0]) == 2:
